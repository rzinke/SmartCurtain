...
def activate_curtain(cnx, cursor, new_position_steps):
	...
	if total_steps - 5 <= new_position_steps:
		SetCurtain.open_curtain(False ^ direction(cursor))
		return new_position(cnx, cursor, total_steps)
	elif new_position_steps <= 5:
		SetCurtain.close_curtain(True ^ direction(cursor))
	...
	remaining_steps = SetCurtain.move_curtain(move_direction, steps_to_move, stop_pin)
